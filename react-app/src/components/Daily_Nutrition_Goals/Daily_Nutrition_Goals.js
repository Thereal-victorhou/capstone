...
import {
	...
	updateCurrentDng,
	deleteUserDng,
} from '../../store/daily_nutrition_goals';
...
const DailyNutritionGoals = () => {
	...
	const [calories, setCalories] = useState(
		currentGoal ? currentGoal?.calories : ''
	);
	const [carbohydrates, setCarbohydrates] = useState(
		currentGoal ? currentGoal?.carbohydrates : ''
	);
	const [fat, setFat] = useState(currentGoal ? currentGoal?.fat : '');
	const [protein, setProtein] = useState(
		currentGoal ? currentGoal?.protein : ''
	);
	...
	useEffect(() => {
		...
		dispatch,
		currentGoal ? currentGoal.calories : calories,
	...
	useEffect(() => {
		...
		if (carbBool) {
			if (carbohydrates && carbohydrates.length > 0) {
				if (calories && carbohydrates * 4 > calories) {
					errArr.push({
						msg: 'Carbohydrates cannot exceed calories',
						type: 'carbohydrates',
					});
					setErrors(errArr);
				} else {
					errArr.forEach((obj) =>
						errArr.splice(errArr.indexOf(obj.type === 'carbohydrates'), 1)
					);
					setErrors(errArr);
				}
		...
		if (fatBool) {
			if (fat && fat.length > 0) {
				if (calories && fat * 9 > calories) {
					...
					// errArr.pop()
					errArr.forEach((obj) =>
						errArr.splice(errArr.indexOf(obj.type === 'fat'), 1)
					);
					setErrors(errArr);
				}
			} else if (!fat) {
		...
		if (proBool) {
			if (protein && protein.length > 0) {
				if (calories && protein * 4 > calories) {
					errArr.push({
						msg: `Protein cannot exceed calories`,
						type: 'protein',
					});
					setErrors(errArr);
				} else {
					errArr.forEach((obj) =>
						errArr.splice(errArr.indexOf(obj.type === 'protein'), 1)
					);
					setErrors(errArr);
				}
			} else if (!protein) {
		...
		if (calories && carbohydrates && fat && protein) {
			if (carbohydrates * 4 + fat * 9 + protein * 4 > calories) {
				errArr.push({
					msg: `Sumn of macros cannot exceed calories`,
					type: 'total',
				});
			} else {
				errArr.forEach((obj) =>
					errArr.splice(errArr.indexOf(obj.type === 'total'), 1)
				);
				setErrors(errArr);
			}
		}
			...
			<div className='dng-main'>
				<div className='dng-form-container'>
					...
					<form className='dng-form' autoComplete='off'>
						...
						<div className='dng-container'>
							<label
								className='dng-carbohydrates-label'
